...
namespace SpotifyApp
{
    ...
    public class Player
    {
        ...
        {
            return LocalStorage.GetTokenData(LocalStorage.ConfigFiles.SPOTIFYSETTINGS) as TokenData;
        }
        public ConfigData GetConfigData()
        {
            return LocalStorage.GetTokenData(LocalStorage.ConfigFiles.SPOTIFYCONFIG) as ConfigData;
        }
        void SaveTokenData(TokenData td)
        {
            string tokenText = JsonSerializer.Serialize(td);
            LocalStorage.SaveTokenDataPublic(tokenText, LocalStorage.ConfigFiles.SPOTIFYSETTINGS);
        }
        void SaveConfigData(ConfigData cd)
        {
            string tokenText = JsonSerializer.Serialize(cd);
            LocalStorage.SaveTokenDataPublic(tokenText, LocalStorage.ConfigFiles.SPOTIFYCONFIG);
        }
